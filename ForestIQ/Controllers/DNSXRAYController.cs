...
namespace ForestIQ.Controllers
{
    ...
    public class DNSXRAYController : ControllerBase
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(filter.DnsServer) || string.IsNullOrWhiteSpace(filter.ZoneName))
                return BadRequest(ApiResponse<object>.Fail("DnsServer and ZoneName are required."));
        ...
        {
            if (string.IsNullOrWhiteSpace(filter.ZoneName))
                return BadRequest(ApiResponse<object>.Fail("ZoneName is required."));
